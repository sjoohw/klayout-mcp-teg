...
이 문서는 process/drawing/measurement 계약과 production 승격 조건을 설명한다.

## 공통 안전 원칙

- 원본 GDS/OAS와 reference는 변경하지 않는다.
- Output은 새 경로에만 쓰며 같은 이름을 덮어쓰지 않는다.
...
Repository는 fabrication process profile을 번들하지 않는다. Geometry source가 PDK에 있을
것으로 추정하지 않으며 [onboarding](../onboarding.md)에서 실제 근거를 선택한다.

### Organization preset

회사 내에서 고정되는 terminal naming/order, 지원 측정 방식, verification environment와
transistor context 기본값을 저장한다. DRC/LVS/PEX를 선택한 job에서는 engine을 available로
간주할 수 있지만 신규 device coverage는 job별로 확인한다. 선택하지 않은 verification은 drawing을
막지 않는다. 실제 production 실행 가능성은 별도 조직 workflow가 판단한다.
...
Single-transistor measurement의 조직 기본값은 다음과 같다.
...
- Output은 host-controlled root 안의 basename만 허용한다.
- Atomic write는 same-directory short `.tmp-UUID`와 replace를 사용한다.
