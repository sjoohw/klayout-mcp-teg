...
| Pad/corpus onboarding | Access metal이 macro edge tolerance 안에 실제 닿을 때만 landing으로 인정한다. Overlay는 stage file을 fresh reload해 source fingerprint를 확인한 뒤 publish한다. Labeled corpus에는 coverage/style/variation/logical-validation score가 있다. | 검증 DUT도 같은 보존 GDS와 metadata에 보여서 비밀 holdout이 아니다. Corpus similarity는 dependency-complete PCell, electrical equivalence 또는 foundry qualification이 아니다. |
| Persistent host state | Technology registry는 package/lifecycle JSON을 시작할 때 hash 검증 후 다시 읽는다. Deployment TOML의 workflow/output root를 status와 onboarding도 함께 사용하며, engine 0개인 doctor는 실패한다. | 재시작 가능한 storage가 실제 process engine·transistor compiler·approval backend를 대신하지 않는다. |
